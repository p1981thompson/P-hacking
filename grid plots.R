...
##NOTE: THIS VERSION HAS PERCENTAGE ON THE Y AXIS OF THE GRID OF PLOTS - REQUEST FROM REVIEWER (D.LAKENS)
...
dd$N<-factor(dd$n) # change variable type to factor so that plotting is correct.
  ...
  dd1.1 <- dd1[dd1$meanA==0.0, ]#subset data for plots A in grid
  dd1.2 <- dd1[dd1$meanA==0.3, ]#subset data for plots B in grid

  dd2 <- dd[dd$vars %in% levels(dd$vars)[2], ]#subset data for plots c and D in grid
  dd2.1 <- dd2[dd2$meanA==0.0, ]#subset data for plots c in grid
  dd2.2 <- dd2[dd2$meanA==0.3, ]#subset data for plots d in grid

  perc.fun<-function(v,n){as.vector(v/rep(unname(tapply(v, (seq_along(v)-1) %/% n, sum)),each=n)*100)} #FUNCTION TO CALCULATE THE PRECENTAGES INSTEAD OF COUNTS FOR THE Y AXIS OF PLOT.
  
dd1.1$Percentage<-perc.fun(dd1.1$count,10) #PERCENTAGES FOR PLOT A
dd1.2$Percentage<-perc.fun(dd1.2$count,10) #PERCENTAGES FOR PLOT B
dd2.1$Percentage<-perc.fun(dd2.1$count,10) #PERCENTAGES FOR PLOT C
dd2.2$Percentage<-perc.fun(dd2.2$count,10) #PERCENTAGES FOR PLOT D
...
plot11 <- ggplot(dd1.1, aes(x = as.numeric(bin), y = Percentage,col=Correlation,lty=N)) +
...
plot1 <- ggplot(dd1.1, aes(x = as.numeric(bin), y = Percentage,col=Correlation,lty=N)) +
  geom_line(lwd = 1) +theme_bw()+ theme(legend.position="none", axis.text.x=element_blank(),axis.title.x=element_blank(),axis.ticks=element_blank(),text = element_text(size=14, face="bold")) + scale_y_continuous(limits = c(0, 15)) + ggtitle("(A)     3 variables     ") 
...
plot2 <- ggplot(dd1.2, aes(x = as.numeric(bin), y = Percentage,col=Correlation,lty=N)) + scale_y_continuous(limits = c(0, 100)) + 
...
plot3 <- ggplot(dd2.1, aes(x = as.numeric(bin), y = Percentage,col=Correlation,lty=N)) +
  geom_line(lwd = 1) +theme_bw()+ theme(legend.position="none",axis.ticks=element_blank()) + 
  scale_y_continuous(limits = c(0, 15)) + ggtitle("(C)     8 variables     ") + scale_x_continuous(breaks=1:10, labels=levels(dp2.2$bin)[1:10])+ theme(axis.text.x = element_text(angle = 45, hjust = 1),text = element_text(size=14,face="bold"))+xlab("Bin")

#This is the plot for the top left panel of the grid (D).
plot4 <- ggplot(dd2.2, aes(x = as.numeric(bin), y = Percentage,col=Correlation,lty=N)) +
  geom_line(lwd = 1) +theme_bw()+ theme(legend.position="none",axis.ticks=element_blank()) + scale_y_continuous(limits = c(0, 100)) +
...
grid.newpage()
# grid.arrange(main.g, sub.sg, rectGrob(), 
...
dp1.1$Percentage<-perc.fun(dp1.1$count,10)#PERCENTAGES FOR PLOT A
dp1.2$Percentage<-perc.fun(dp1.2$count,10)#PERCENTAGES FOR PLOT B
dp2.1$Percentage<-perc.fun(dp2.1$count,10)#PERCENTAGES FOR PLOT C
dp2.2$Percentage<-perc.fun(dp2.2$count,10)#PERCENTAGES FOR PLOT D

#This plot is generated as a dummy, so that the legend info can be stripped from it and repositioned in the main grid of plots.
plot21 <- ggplot(dp1.1, aes(x = as.numeric(bin), y = Percentage,col=Correlation,lty=N)) +
  geom_line(lwd = 1)  +theme_bw() + theme(legend.position="top") +
  theme(text = element_text(size=14),legend.key = element_rect(colour = NA),legend.text = element_text(size = 14, face = "bold"),legend.title = element_text(size=14, face="bold"))

#This is the plot for the top left panel of the grid (A). We remove the X axis labels here to conserve space and unclutter the plot. limits are manually set so that the plots are on the same scales as far as possible for comparison.
plot1A <- ggplot(dp1.1, aes(x = as.numeric(bin), y = Percentage,col=Correlation,lty=N)) +
  geom_line(lwd = 1) +theme_bw()+ theme(legend.position="none", axis.text.x=element_blank(),axis.title.x=element_blank(),axis.ticks=element_blank(),text = element_text(size=14, face="bold")) + scale_y_continuous(limits = c(0, 15)) + ggtitle("(A)     3 variables     ") 

#This is the plot for the top left panel of the grid (B). Again, We remove the X axis labels here to conserve space and unclutter the plot. limits are manually set so that the plots are on the same scales as far as possible for comparison.
plot2A <- ggplot(dp1.2, aes(x = as.numeric(bin), y = Percentage,col=Correlation,lty=N)) + scale_y_continuous(limits = c(0, 100)) + 
  geom_line(lwd = 1) +theme_bw()+ theme(legend.position="none", axis.text.x=element_blank(),axis.title.x=element_blank(),axis.ticks=element_blank(),text = element_text(size=14, face="bold")) + ggtitle("(B)     3 variables     ")

#This is the plot for the top left panel of the grid (C).
plot3A <- ggplot(dp2.1, aes(x = as.numeric(bin), y = Percentage,col=Correlation,lty=N)) +
  geom_line(lwd = 1) +theme_bw()+ theme(legend.position="none",axis.ticks=element_blank()) + 
  scale_y_continuous(limits = c(0, 15)) + ggtitle("(C)     8 variables     ") + scale_x_continuous(breaks=1:10, labels=levels(dp2.2$bin)[1:10])+ theme(axis.text.x = element_text(angle = 45, hjust = 1),text = element_text(size=14,face="bold"))+xlab("Bin")

#This is the plot for the top left panel of the grid (D).
plot4A <- ggplot(dp2.2, aes(x = as.numeric(bin), y = Percentage,col=Correlation,lty=N)) +
  geom_line(lwd = 1) +theme_bw()+ theme(legend.position="none",axis.ticks=element_blank()) + scale_y_continuous(limits = c(0, 100)) +
  ggtitle("(D)     8 variables     ") + scale_x_continuous(breaks=1:10, labels=levels(dp2.2$bin)[1:10])+ theme(axis.text.x = element_text(angle = 45, hjust = 1),text = element_text(size=14, face="bold"))+xlab("Bin")
...
#Function to arrange plots in correct layout
#####################################################################
windows(width=10, height=10) #opens a plot window
